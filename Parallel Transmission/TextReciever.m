...
while (1)       % Infinite while loop to receive bits
    voltage1= (readVoltage(d, 'A0'));    % Read voltae from  Analog pin  A0
    voltage2= (readVoltage(d, 'A1'));    % Read voltae from  Analog pin  A1
    disp(voltage1);      % Display the received voltage in float
        if (voltage1 >= 1 && voltage2 >= 1)   % Condition to predict whether 1 or 0 is transmitted 
...
while (1) 
    for i=1:1:4
        ...
        if (voltage1 >= 1 && voltage1 >= 1)   % Condition to predict whether 1 or 0 is transmitted 
            temp1 ='1';
            temp2 = '1';
            pause(2);
            
        elseif(voltage1 >= 1 && voltage1 >= 0)
            temp1 ='1';
            temp2 = '0';
            pause(2); 
            
        elseif(voltage1 >= 0 && voltage1 >= 1)
            temp1 ='0';
            temp2 = '1';
            pause(2);  
         
        elseif(voltage1 >= 0 && voltage1 >= 0)
            temp1 ='1';
            temp2 = '0';
            pause(2);  
